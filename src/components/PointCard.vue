  ...
  <q-card :class="{ 'card card__is-active': isActive }" flat bordered>
    <q-card-section>
      <div class="row items-center">
        <q-checkbox v-model="selected" color="accent" @update:model-value="onSelect" />
        <div class="col-auto text-subtitle1 q-mr-sm">Точка №{{ point.id }} ({{ point.code }})</div>
        <div class="col">
          <div class="text-caption">Код: {{ point.code }}</div>
        </div>
        <q-space />
        <q-btn flat round icon="more_vert" class="col-auto" />
      </div>
